        ...
        embed
            ...
            .author(|a| {
                ...
                let (name, avatar_url) = match owner_id.find() {
                    ...
                    None => match rest::get_user(owner_id.0) {
...
impl Config {
    ...
    fn load(&mut self) {
        ...
        self.tags = serde_json::from_str(&tags)
    ...
    fn save(&self) {
        ...
        let mut file = File::create(&temp)
            .expect(&format!("Failed to create file: {}", temp));
        file.write_all(
...
impl Tags {
    fn get_possible_tags(&self, guild: Option<GuildId>)
        -> HashMap<String, Tag>
    {
    ...
    fn get_tag(&self, guild: Option<GuildId>, name: String)
        -> Result<Tag, String>
    {
    ...
    fn put_tag(&self, guild: Option<GuildId>, name: String, tag: Tag) {
        ...
        let mut config = TAGS.config.lock()
    ...
    -> Result<(), String>
{
    ...
    database.insert(name.clone(), Tag::new(
            name.clone(),
            content,
            message.author.id.0,
            None,
...
pub fn info(context: &Context, message: &Message, args: Vec<String>)
    -> Result<(), String>
{
    let mut args = args.into_iter();
    ...
    check_msg(context.send_message(
        message.channel_id,
        |m| m.embed(|e| tag.as_embed(e)),
    ));

    Ok(())
}

pub fn list(context: &Context, message: &Message, _args: Vec<String>)
    -> Result<(), String>
{
    // TODO: ensure we get the GuildId even if it's not in the cache.
    let guild_id = message.guild_id();
...
pub fn edit(context: &Context, message: &Message, args: Vec<String>)
    -> Result<(), String>
{
    let mut args = args.into_iter();
...
pub fn delete(context: &Context, message: &Message, args: Vec<String>)
    -> Result<(), String>
{
    let mut args = args.into_iter();
